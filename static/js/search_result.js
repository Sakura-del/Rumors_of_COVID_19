...
$.ajax({
    ...
    success: function (result) {
        ...
        for (var i = 0; i < 10 && i < result["retlist"].length; i++) {
            ...
            new_h3.innerHTML = result["retlist"][i]["title"];
            ...
            var new_p = document.createElement("p");
            new_p.innerHTML = result["retlist"][i]["markstyle"];
            new_col_sm_9_div.appendChild(new_p);

            var new_p = document.createElement("p");
            new_p.className = "text-muted";
            var tmptag;
            for (var j = 0; j < result["retlist"][i]["tag"].length; j++) {
                if (j == 0) {
            ...
            newimg.src = result["retlist"][i]["coversqual"];
