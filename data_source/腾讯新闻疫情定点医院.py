...
def QQ_news_designated_hospital_func():
    designated_hospital_list = []
    def Province_designated_hospital_func(province):
        ...
        designated_hospital_list.append(response['args']['rsp']['info'])


    thread_list = []
    for province in ['北京','天津','河北','山西','内蒙古','辽宁','吉林','黑龙江','上海','江苏','浙江','安徽','福建','江西','山东','河南','湖北','湖南','广东','广西','海南','重庆','四川','贵州','云南','西藏','陕西','甘肃','青海','宁夏','新疆']:
        thread = Thread(target = Province_designated_hospital_func,args = (province,))
        thread.start()
    ...
    with open('data_source/data_from_creeper/腾讯新闻疫情定点医院.json', 'w', encoding='utf-8') as file:
        file.write(json.dumps(designated_hospital_list, ensure_ascii=False))



func_list = [QQ_news_designated_hospital_func]